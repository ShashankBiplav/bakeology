...
exports.postRecipe = async(req, res, next)=>{
    ...
    if (req.file){
        imageUrl = req.file.path;//generated by multer that holds path to image
    }
    if (!req.file) {
    ...
    if(actualChef._id.toString() !== req.userId){
        const error = new Error('Not Authorized.');
        error.statusCode = 403;
        return next(error);
    }
    if (actualChef.isApproved !== true){
        const error = new Error('Chef not Approved.');
        error.statusCode = 403;
        return next(error);
    }
...
exports.updateRecipe = async (req, res, next) =>{
    ...
    try{
        ...
        if (recipe.chef.isApproved !== true){
            const error = new Error('Chef not Approved.');
            error.statusCode = 403;
            throw error;
        }
